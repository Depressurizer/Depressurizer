...
//     This file (GameDB.cs) is part of Depressurizer.
...
namespace Depressurizer
{
	...
	public class GameDBEntry
	{
		...
		private AppType ScrapeStoreHelper(int id)
		{
			...
			try
			{
				...
				while ((resp.StatusCode == HttpStatusCode.Found) && (count < 5))
				{
					...
					if (resp.Headers[HttpResponseHeader.Location] == Resources.UrlSteamStore)
					{
						...
						Logger.Instance.Verbose(GlobalStrings.GameDB_ScrapingRedirectedToMainStorePage, id);
				...
				else if (resp.ResponseUri.Segments[1] == "agecheck/")
				{
					...
					{
						// Age check + redirect
						Logger.Instance.Verbose(GlobalStrings.GameDB_ScrapingHitAgeCheck, id, resp.ResponseUri.Segments[3].TrimEnd('/'));
				...
				else if (resp.ResponseUri.Segments[2].TrimEnd('/') != id.ToString())
				{
					...
					Logger.Instance.Verbose(GlobalStrings.GameDB_ScrapingRedirectedToOtherApp, id, resp.ResponseUri.Segments[2].TrimEnd('/'));
			...
			else if (regGamecheck.IsMatch(page) || regSoftwarecheck.IsMatch(page))
			{
				...
				{
					Logger.Instance.Verbose(GlobalStrings.GameDB_ScrapingParsedDLC, id, string.Join(",", Genres));
					result = AppType.DLC;
				}
				else
				{
					Logger.Instance.Verbose(GlobalStrings.GameDB_ScrapingParsed, id, string.Join(",", Genres));
	...
	public class GameDB
	{
		...
		public StoreLanguage dbLanguage
		{
			get => _dbLanguage;
			set
			{
		...
		public void Load(string path, bool compress)
		{
			...
			try
			{
				...
				Logger.Instance.Info("GameDB XML processed, load complete. Db Language: " + dbLanguage);
			}
			finally
			{
				stream?.Close();
