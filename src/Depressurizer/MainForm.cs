    ...
    public partial class FormMain : MaterialForm
    {
        ...
        public class MyRenderer : ToolStripRenderer
        {
            protected override void OnRenderMenuItemBackground(ToolStripItemRenderEventArgs e)
            {
                ...
                var colorText = e.Item.Selected ? Color.FromArgb(255, 255, 255) : Color.FromArgb(169, 167, 167);
                if (e.ToolStrip is ToolStripDropDown)
                {
                    var colorItem = Color.FromArgb(55, 71, 79);
                    using (var brush = new SolidBrush(colorItem))
                    {
                ...
                {
                    var colorItem = Color.FromArgb(38, 50, 56);
                    using (var brush = new SolidBrush(colorItem))
                    {
                        e.Graphics.FillRectangle(brush, rc);
                    }
            ...
            protected override void OnRenderToolStripBorder(ToolStripRenderEventArgs e)
            {
                if (e.ToolStrip is ToolStripDropDown)
                {
                    var p = new Pen(Color.FromArgb(41, 42, 46));
                ...
                using (var p = new Pen(Color.FromArgb(41, 42, 46)))
                {
        ...
        private void InitializeLstGames()
        {
            ...
            colYear.AspectGetter = delegate(object g)
            {
                ...
                var culture = Utility.GetCultureInfoFromStoreLanguage(Program.GameDB.dbLanguage);
            ...
            {
                if (g == null) return "";
                var platforms = Program.GameDB.Games[((GameInfo) g).Id].Platforms;
                return (platforms & AppPlatforms.Linux) != 0 && platforms != AppPlatforms.All
                    ? platforms + ", SteamOS"
                    : platforms.ToString();
            };
            ...
            colReviewLabel.AspectGetter = delegate(object g)
            {
                ...
                {
                    var reviewTotal = Program.GameDB.Games[id].ReviewTotal;
                    var reviewPositivePercentage = Program.GameDB.Games[id].ReviewPositivePercentage;
            ...
            {
                var index = (int) obj;
                var reviewLabels = new Dictionary<int, string>
                {
            ...
            {
                var LastPlayed = (DateTime) obj;
                var threadForCulture = new Thread(delegate() { });
                var format = threadForCulture.CurrentCulture.DateTimeFormat.ShortDatePattern;
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            if (!Utility.IsOnScreen(this)) Location = new Point(0, 0);
            ...
            if (Settings.Instance.UpdateAppInfoOnStart) UpdateGameDBFromAppInfo();
            var aWeekInSecs = 7 * 24 * 60 * 60;
            ...
            if (Settings.Instance.CheckForDepressurizerUpdates) CheckForDepressurizerUpdates();
        ...
        private void UpdateGameDBFromAppInfo()
        {
            try
            {
                var num = Program.GameDB.UpdateFromAppInfo(
                    string.Format(Resources.AppInfoPath, Settings.Instance.SteamPath));
                AddStatus(string.Format(GlobalStrings.MainForm_Status_AppInfoAutoupdate, num));
                if (num > 0 && Settings.Instance.AutosaveDB) SaveGameDB();
            }
            catch (Exception e)
            {
        ...
        private void UpdateGameDBFromHltb()
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (dlg.Updated > 0 && Settings.Instance.AutosaveDB) SaveGameDB();
                }
            }
        ...
        private void CreateProfile()
        {
            ...
            if (res == DialogResult.OK)
            {
                ...
                if (dlg.DownloadNow) UpdateLibrary();

                if (dlg.ImportNow) ImportConfig();
                if (dlg.SetStartup)
                {
        ...
        private void LoadProfile()
        {
            ...
            var res = dlg.ShowDialog();
            if (res == DialogResult.OK) LoadProfile(dlg.FileName, false);
        }
        ...
        private void LoadProfile(string path, bool checkForChanges = true)
        {
            ...
            if (CurrentProfile.AutoUpdate) UpdateLibrary();
            if (CurrentProfile.AutoImport) ImportConfig();
            Cursor = Cursors.Default;
        ...
        private void SaveProfileAs()
        {
            ...
            var res = dlg.ShowDialog();
            if (res == DialogResult.OK) SaveProfile(dlg.FileName);
        }
        ...
        private bool SaveProfile(string path = null)
        {
            ...
            if (CurrentProfile.AutoExport) ExportConfig();
        ...
        private void UpdateLibrary()
        {
            ...
            if (CurrentProfile.LocalUpdate)
                ...
                {
                    var newApps = 0;
                    var appFilter = CurrentProfile.IncludeUnknown
                        ? AppTypes.InclusionUnknown
                        : AppTypes.InclusionNormal;
                    var totalApps = CurrentProfile.GameData.UpdateGameListFromOwnedPackageInfo(CurrentProfile.SteamID64,
            ...
            else if (CurrentProfile.WebUpdate)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (updateDlg.Failover) AddStatus(GlobalStrings.MainForm_XMLDownloadFailed);
        ...
        private void ImportConfig()
        {
            ...
            try
            {
                var count = CurrentProfile.ImportSteamData();
        ...
        {
            if (!ProfileLoaded) return;
            var dlg = new DlgAutoCat(CurrentProfile.AutoCats, CurrentProfile.GameData, selected,
                CurrentProfile.FilePath);
        ...
        private void SaveFilter()
        {
            ...
            var dlg = new GetStringDlg(cboFilter.Text, GlobalStrings.MainForm_SaveFilter,
        ...
        private Category CreateCategory()
        {
            ...
            {
                var newCat = CurrentProfile.GameData.AddCategory(dlg.Value);
                if (newCat != null)
                {
        ...
        private void DeleteCategory()
        {
            ...
            {
                var c = item.Tag as Category;
                if (c != null && c != CurrentProfile.GameData.FavoriteCategory) toDelete.Add(c);
            }
            ...
            if (toDelete.Count > 0)
            {
                ...
                res = MessageBox.Show(
                    toDelete.Count == 1
                        ? string.Format(GlobalStrings.MainForm_DeleteCategory, toDelete[0].Name)
                        : string.Format(GlobalStrings.MainForm_DeleteCategoryMulti, toDelete.Count),
                    GlobalStrings.DBEditDlg_Confirm, MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                if (res == DialogResult.Yes)
                {
                    var deleted = 0;
        ...
        private bool RenameCategory()
        {
            ...
            {
                var c = lstCategories.SelectedItems[0].Tag as Category;
                if (c != null && c != CurrentProfile.GameData.FavoriteCategory)
                {
                    var dlg = new GetStringDlg(c.Name,
                    ...
                    if (dlg.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        {
                            var newCat = CurrentProfile.GameData.RenameCategory(c, newName);
                            if (newCat != null)
                            {
                                FillAllCategoryLists();
                                RebuildGamelist();
                                MakeChange(true);
                                for (var index = 2; index < lstCategories.Items.Count; index++)
        ...
        {
            var dlg = new DlgGame(CurrentProfile.GameData, null);
            if (dlg.ShowDialog() == DialogResult.OK)
            {
        ...
        private void EditGame()
        {
            ...
            {
                var g = tlstGames.SelectedObjects[0];
                var dlg = new DlgGame(CurrentProfile.GameData, g);
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    Cursor.Current = Cursors.WaitCursor;
        ...
        {
            var selectCount = lstGames.SelectedObjects.Count;
            if (selectCount > 0)
                if (MessageBox.Show(
                    ...
                    == DialogResult.Yes)
                {
                    ...
                    var ignored = 0;
                    var removed = 0;
                    foreach (var g in tlstGames.SelectedObjects)
                    {
                        g.ClearCategories(true);
                        if (CurrentProfile.GameData.Games.Remove(g.Id)) removed++;
        ...
        private void AddCategoryToSelectedGames(Category cat, bool forceClearOthers)
        {
            if (lstGames.SelectedObjects.Count > 0)
            {
                ...
                FillAllCategoryLists();
                if (forceClearOthers) FilterGamelist(false);
        ...
        private void RemoveCategoryFromSelectedGames(Category cat)
        {
            ...
            {
                Cursor.Current = Cursors.WaitCursor;
                foreach (var g in tlstGames.SelectedObjects) g.RemoveCategory(cat);
                FillAllCategoryLists();
        ...
        private void AssignFavoriteToSelectedGames(bool fav)
        {
            ...
            {
                Cursor.Current = Cursors.WaitCursor;
                foreach (var g in tlstGames.SelectedObjects) g.SetFavorite(fav);
        ...
        private void AssignHiddenToSelectedGames(bool hidden)
        {
            ...
            {
                Cursor.Current = Cursors.WaitCursor;
                foreach (var g in tlstGames.SelectedObjects) g.SetHidden(hidden);
                FillCategoryList(false);
                FilterGamelist(false);
                MakeChange(true);
                Cursor.Current = Cursors.Default;
        ...
        private int AutoCatGameCount()
        {
            ...
            {
                foreach (var g in tlstGames.Objects)
                    if (g.Id > 0)
                        count += 1;
            }
            else
            {
                if (CurrentProfile != null)
                    foreach (var g in CurrentProfile.GameData.Games.Values)
                        if (g != null && g.Id > 0)
                            count += 1;
            }
        ...
        private void Autocategorize(bool selectedOnly, AutoCat autoCat, bool scrape = true, bool refresh = true)
        {
            ...
            var notInDbOrOldData = new Queue<int>();
            var oldDbDataCount = 0;
            var notInDbCount = 0;
            ...
            if ((notInDbCount > 0 || oldDbDataCount > 0) && scrape)
            {
                ...
                if (MessageBox.Show(message, GlobalStrings.DBEditDlg_Confirm, MessageBoxButtons.YesNo,
                    ...
                    == DialogResult.Yes)
                {
                    var scrapeDlg = new DbScrapeDlg(notInDbOrOldData);
                    var scrapeRes = scrapeDlg.ShowDialog();
                    ...
                    else
                    {
                        ...
                        if (scrapeDlg.JobsCompleted > 0 && Settings.Instance.AutosaveDB) SaveGameDB();
                    }
                }
            ...
            {
                var res = autoCat.CategorizeGame(g, CurrentProfile.GameData.GetFilter(autoCat.Filter));
                if (res == AutoCatResult.Success) updated++;
            }
        ...
        {
            var res = MessageBox.Show(GlobalStrings.MainForm_OverwriteExistingNames,
        ...
        {
            var count = CurrentProfile.GameData.RemoveEmptyCategories();
        ...
        private void UpdateTitle()
        {
            ...
            if (unsavedChanges) sb.Append(" *");
            Text = sb.ToString();
            //update Avatar picture for new profile
            if (CurrentProfile != null) picAvatar.Image = CurrentProfile.GetAvatar();
        }
        ...
        private void SelectFilter(Settings settings)
        {
            if (settings.Filter != string.Empty)
                for (var i = 0; i < cboFilter.Items.Count; i++)
                {
                    var name = cboFilter.GetItemText(cboFilter.Items[i]);
        ...
        private void SelectAutoCats(Settings settings)
        {
            ...
            {
                var autocats = settings.AutoCats.Split(',').ToList();
                foreach (var ac in autocats)
                    for (var i = 0; i < lvAutoCatType.Items.Count; i++)
        ...
        {
            var settings = Settings.Instance;
            var autocats = string.Empty;
            for (var i = 0; i < lvAutoCatType.CheckedItems.Count; i++)
        ...
        {
            var gamelist = lstGames.Objects.Cast<GameInfo>().ToList();
        ...
        private void FillGameList()
        {
            ...
            if (gamelist.Count > 0) StartBannerThread(new List<GameInfo>(gamelist));
        ...
        private void FillAllCategoryLists()
        {
            ...
            foreach (var c in CurrentProfile.GameData.Categories)
                if (c != CurrentProfile.GameData.FavoriteCategory)
                {
                    var item = contextGame_AddCat.DropDownItems.Add(c.Name);
                    item.Tag = c;
                    ...
                    var listItem = new ListViewItem(c.Name);
                    listItem.Tag = c;
        ...
        {
            var selected = lstCategories.SelectedItems.Count > 0 ? lstCategories.SelectedItems[0].Tag : null;
            var selectedIndex = lstCategories.SelectedItems.Count > 0 ? lstCategories.SelectedIndices[0] : -1;
            ...
            {
                var i = new ListViewItem(GlobalStrings.MainForm_All + " (" +
        ...
        private void UpdateGameCheckStates()
        {
            lstMultiCat.BeginUpdate();
            var first = true;
            foreach (ListViewItem item in lstMultiCat.Items) item.StateImageIndex = 0;
            ...
            else
            {
                ...
                contextGameRemCat.Items.Clear();
                foreach (var game in tlstGames.SelectedObjects)
                    if (game != null)
                    {
        ...
        private void AddRemoveCategoryContextMenu(GameInfo game)
        {
            foreach (var c in game.Categories)
            {
                ...
                if (!found)
                {
                    var item = contextGameRemCat.Items.Add(c.Name);
                    item.Tag = c;
                    item.Click += contextGameRemCat_Category_Click;
                }
            }
        ...
        private void FillAutoCatLists()
        {
            ...
            var selected = cmbAutoCatType.SelectedItem;
            ...
            if (CurrentProfile != null)
                foreach (var ac in CurrentProfile.AutoCats)
                    if (ac != null)
                    {
                        // Fill main screen dropdown
                        var listItem = new ListViewItem(ac.DisplayName);
                        listItem.Tag = ac;
                        ...
                        // Fill main menu list
                        var item = menu_Tools_Autocat_List.Items.Add(ac.DisplayName);
                        item.Tag = ac;
                        item.Name = ac.Name;
                        item.Click += menuToolsAutocat_Item_Click;
                    }
        ...
        {
            var gamesSelected = lstGames.SelectedObjects.Count > 0;
        ...
        private void UpdateEnabledStatesForCategories()
        {
            ...
            foreach (ListViewItem item in lstCategories.SelectedItems)
            {
                ...
                if (c != null && !(CurrentProfile != null && c == CurrentProfile.GameData.FavoriteCategory)) break;
                c = null;
            }
        ...
        {
            var sCat = Settings.Instance.SingleCatMode;
        ...
        {
            if (contextGame.IsDropDown) SendKeys.SendWait(e.Delta > 0 ? BIG_UP : BIG_DOWN);
        }
        ...
        private void FormMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (e.CloseReason == CloseReason.UserClosing) e.Cancel = !CheckForUnsaved();
        }
        ...
        {
            var clientPoint = lstCategories.PointToClient(new Point(x, y));
        ...
        private void lstCategories_DragDrop(object sender, DragEventArgs e)
        {
            ...
            {
                lstCategories.SelectedIndices.Clear();
                if (dragOldCat >= 0) lstCategories.SelectedIndices.Add(dragOldCat);
                isDragging = false;
                ClearStatus();
                var dropItem = GetCategoryItemAtPoint(e.X, e.Y);

                SetDragDropEffect(e);

                if (dropItem.Tag != null && dropItem.Tag is Category)
                {
                    var dropCat = (Category) dropItem.Tag;
        ...
        {
            var selectedGames = new int[lstGames.SelectedObjects.Count];
            for (var i = 0; i < lstGames.SelectedObjects.Count; i++) selectedGames[i] = tlstGames.SelectedObjects[i].Id;
        ...
        private void lstCategories_DragOver(object sender, DragEventArgs e)
        {
            if (isDragging)
            {
                ...
                lstCategories.SelectedIndices.Clear();
                var overItem = GetCategoryItemAtPoint(e.X, e.Y);
        ...
        {
            ClearStatus();
            var close = new DlgClose(GlobalStrings.MainForm_SaveProfileConfirm, GlobalStrings.MainForm_SaveProfile,
                SystemIcons.Question.ToBitmap(), false, CurrentProfile.AutoExport);
            var res = close.ShowDialog();
            if (res == DialogResult.Yes)
            {
        ...
        private void menu_Profile_EditAutoCats_Click(object sender, EventArgs e)
        {
            ...
            if (lvAutoCatType.Items.Count > 0) selected = (AutoCat) lvAutoCatType.Items[0].Tag;
        ...
        {
            var dlg = new DBEditDlg(CurrentProfile != null ? CurrentProfile.GameData : null);
            dlg.ShowDialog();
            LoadGameDB();
        }
        ...
        private void menu_Tools_Settings_Click(object sender, EventArgs e)
        {
            ...
            var actualCulture = Thread.CurrentThread.CurrentUICulture;
            ...
            if (actualCulture.Name != Thread.CurrentThread.CurrentUICulture.Name)
            {
                ...
                var actualSplitDistanceMain = splitContainer.SplitterDistance;
                var actualSplitDistanceSecondary = splitGame.SplitterDistance;
                var actualSplitDistanceCategories = splitCategories.SplitterDistance;
                var actualSplitDistanceBrowser = splitBrowser.SplitterDistance;
        ...
        {
            var selectedCat = lstCategories.SelectedItems.Count > 0 && lstCategories.SelectedItems[0].Tag != null;
        ...
        {
            var selectedGames = lstGames.SelectedObjects.Count > 0;
        ...
        {
            if (lstGames.SelectedObjects.Count > 0) Utility.LaunchStorePage(tlstGames.SelectedObjects[0].Id);
        }
        ...
        private void contextCat_SetAdvanced_None_Click(object sender, EventArgs e)
        {
            if (lstCategories.SelectedItems.Count > 0)
            {
                foreach (ListViewItem i in lstCategories.SelectedItems) SetItemState(i, (int) AdvancedFilterState.None);
                OnViewChange();
            }
        }
        ...
        {
            var profilePath = Path.GetDirectoryName(CurrentProfile.FilePath);
            var restore = new DlgRestore(profilePath);

            var res = restore.ShowDialog();
        ...
        {
            var sharedconfigPath = Path.GetDirectoryName(string.Format(Resources.ConfigFilePath,
                Settings.Instance.SteamPath, Profile.ID64toDirName(CurrentProfile.SteamID64)));
            var restore = new DlgRestore(sharedconfigPath);
        ...
        {
            if (AdvancedCategoryFilter) SaveFilter();
        }

        private void mbtnFilterDelete_Click(object sender, EventArgs e)
        {
            if (AdvancedCategoryFilter) DeleteFilter((Filter) cboFilter.SelectedItem);
        }

        private void cboFilter_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cboFilter.SelectedItem != null && AdvancedCategoryFilter) ApplyFilter((Filter) cboFilter.SelectedItem);
        }

        private void mbtnFilterRename_Click(object sender, EventArgs e)
        {
            if (cboFilter.SelectedItem != null && AdvancedCategoryFilter) RenameFilter((Filter) cboFilter.SelectedItem);
        }
        ...
        {
            ClearStatus();
            if (lstGames.SelectedObjects.Count > 0) LaunchGame(tlstGames.SelectedObjects[0]);
            FlushStatus();
        }
        ...
        private void cmdAddCatAndAssign_Click(object sender, EventArgs e)
        {
            ...
            {
                var cat = CurrentProfile.GameData.GetCategory(txtAddCatAndAssign.Text);
        ...
        private void lstCategories_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (!isDragging)
            {
                ...
                if (lstCategories.SelectedItems.Count > 0)
                {
                    var selItem = lstCategories.SelectedItems[0];
        ...
        private void lstCategories_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                ...
                case Keys.Space:
                    if (AdvancedCategoryFilter)
                    {
                        var reverse = ModifierKeys == Keys.Shift;
                        foreach (ListViewItem i in lstCategories.SelectedItems)
        ...
        private void lstCategories_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                var overItem = lstCategories.GetItemAt(e.X, e.Y);
                if (overItem != null)
            ...
            else if (e.Button == MouseButtons.Left)
            {
                if (AdvancedCategoryFilter)
                {
                    var i = lstCategories.GetItemAt(e.X, e.Y);
        ...
        private void RenameFilter(Filter f)
        {
            if (AdvancedCategoryFilter)
            {
                var dlg = new GetStringDlg(f.Name, string.Format(GlobalStrings.MainForm_RenameFilter, f.Name),
                    GlobalStrings.MainForm_EnterNewName, GlobalStrings.MainForm_Rename);
        ...
        {
            var oldState = i.StateImageIndex;
        ...
        private void lstGames_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (lstGames.SelectedObjects.Count > 0)
            {
                ...
                if (webBrowser1.Visible)
                {
                    ...
                    webBrowser1.Navigate(string.Format(Resources.UrlSteamStoreApp + "?l=" + storeLanguage,
                        g.Id));
                }
            }
            else if (webBrowser1.Visible)
            {
                try
                {
                    ...
                    {
                        var g = tlstGames.Objects[0];
                        webBrowser1.ScriptErrorsSuppressed = true;
                        webBrowser1.Navigate(string.Format(
                            Resources.UrlSteamStoreApp + "?l=" + storeLanguage, g.Id));
                    }
                    else
                    {
                        webBrowser1.ScriptErrorsSuppressed = true;
        ...
        private void lstGames_FormatCell(object sender, FormatCellEventArgs e)
        {
            ...
            try
            {
                var decoration = new ImageDecoration(Image.FromFile(bannerFile));
            ...
            if (Program.GameDB.Games.ContainsKey(g.Id) && Program.GameDB.Games[g.Id].Tags != null)
                ...
                {
                    var earlyAccessDecoration = new ImageDecoration(imglistEarlyAccess.Images[0])
                    {
            ...
            var td = new TextDecoration(g.Id.ToString(), ContentAlignment.BottomLeft)
            {
        ...
        {
            var i = lstMultiCat.GetItemAt(e.X, e.Y);
        ...
        private void HandleMultiCatItemActivation(ListViewItem item, bool modKey)
        {
            if (item != null)
            {
                if (item.StateImageIndex == 0 || item.StateImageIndex == 2 && modKey)
                {
                    ...
                    if (cat != null) AddCategoryToSelectedGames(cat, false);
                }
                else if (item.StateImageIndex == 1 || item.StateImageIndex == 2 && !modKey)
                {
                    item.StateImageIndex = 0;
                    var cat = item.Tag as Category;
                    if (cat != null) RemoveCategoryFromSelectedGames(cat);
                }
            }
        }
        ...
        private void ResortToolStripItemCollection(ToolStripItemCollection coll)
        {
            ...
            foreach (ToolStripItem oItem in oAList) coll.Add(oItem);
        }
        ...
        private bool CheckForUnsaved()
        {
            ...
            var close = new DlgClose(GlobalStrings.MainForm_UnsavedChangesWillBeLost,
        ...
        private bool ShouldDisplayGame(GameInfo g)
        {
            ...
            if (AdvancedCategoryFilter) return g.IncludeGame(advFilter);

            if (g.Hidden) return lstCategories.SelectedItems[0].Tag.ToString() == GlobalStrings.MainForm_Hidden;

            if (lstCategories.SelectedItems[0].Tag.ToString() == GlobalStrings.MainForm_All) return true;
        ...
        {
            var installkey = @"SOFTWARE\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_BROWSER_EMULATION";
            var entryLabel = GetType().Assembly.GetName().Name + ".exe";
            ...
            var existingSubKey =
                Registry.LocalMachine.OpenSubKey(installkey, false); // readonly key
                ...
                Convert.ToInt32(existingSubKey.GetValue(entryLabel)) != value)
            {
                ...
                try
                {
                    existingSubKey = Registry.LocalMachine.OpenSubKey(installkey,
                        RegistryKeyPermissionCheck.ReadWriteSubTree); // writable key
        ...
        private void mbtnEditAutocats_Click(object sender, EventArgs e)
        {
            ...
            if (lvAutoCatType.SelectedItems.Count > 0)
                ...
                selected = (AutoCat) lvAutoCatType.CheckedItems[0].Tag;
            else if (lvAutoCatType.Items.Count > 0) selected = (AutoCat) lvAutoCatType.Items[0].Tag;
            EditAutoCats(selected);
            FlushStatus();
        }
        ...
        private void CheckForDepressurizerUpdates()
        {
            ...
            try
            {
                ...
                using (var wc = new WebClient())
                {
                    ...
                    var json = wc.DownloadString(Resources.UrlLatestRelease);
