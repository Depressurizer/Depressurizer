...
namespace Depressurizer
{
    public partial class AutoCatConfigPanel_Curator : AutoCatConfigPanel
    {
        ...
        public void FillRecommendationsList()
        {
            ...
            lstIncluded.Items[1].Tag =
                Enum.GetName(typeof(CuratorRecommendation), CuratorRecommendation.NotRecommended);
        ...
        {
            var ac = autocat as AutoCatCurator;
            if (ac == null) return;

            txtCategoryName.Text = ac.CategoryName;
            txtCuratorUrl.Text = ac.CuratorUrl;

            foreach (var rec in ac.IncludedRecommendations) lstIncluded.Items[rec.GetHashCode() - 1].Checked = true;
        }
        ...
        public override void SaveToAutoCat(AutoCat autocat)
        {
            ...
            foreach (ListViewItem i in lstIncluded.Items)
                if (i.Checked)
                    ac.IncludedRecommendations.Add(
                        (CuratorRecommendation) Enum.Parse(typeof(CuratorRecommendation), i.Tag.ToString()));
        }
