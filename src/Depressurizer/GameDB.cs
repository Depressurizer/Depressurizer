...
namespace Depressurizer
{
    ...
    public class GameDBEntry
    {
        ...
        {
            var useAppInfoFields = other.LastAppInfoUpdate > LastAppInfoUpdate ||
                                   LastAppInfoUpdate == 0 && other.LastStoreScrape >= LastStoreScrape;
            var useScrapeOnlyFields = other.LastStoreScrape >= LastStoreScrape;
        ...
        private static readonly Regex regGamecheck = new Regex(
            @"<a href=""http://store\.steampowered\.com/search/\?term=&snr=",
            RegexOptions.Compiled);

        private static readonly Regex regSoftwarecheck =
            new Regex(@"<a href=""http://store\.steampowered\.com/search/\?category1=994&snr=", RegexOptions.Compiled);

        private static readonly Regex regDLCcheck =
            new Regex(
        ...
        private static readonly Regex regTags = new Regex(@"<a[^>]*class=""app_tag""[^>]*>([^<]*)</a>",
            RegexOptions.Compiled);

        private static readonly Regex regDevelopers =
            new Regex(
        ...
        private static readonly Regex regRelDate =
            new Regex(@"<div class=""release_date"">\s*<div[^>]*>[^<]*<\/div>\s*<div class=""date"">([^<]+)<\/div>",
                RegexOptions.Compiled);
        ...
        private static readonly Regex regReviews =
            new Regex(
                @"<span class=""(?:nonresponsive_hidden ?| responsive_reviewdesc ?){2}"">[^\d]*(\d+)%[^\d]*([\d.,]+)[^\d]*\s*</span>",
                RegexOptions.Compiled);

        private static readonly Regex regAchievements =
            new Regex(
        ...
        private static readonly Regex regVrSupportHeadsetsSection =
            new Regex(
                @"<div class=""details_block vrsupport"">(.*)<div class=""details_block vrsupport"">.*<div class=""details_block vrsupport"">",
                RegexOptions.Compiled);

        private static readonly Regex regVrSupportInputSection =
            new Regex(
                @"<div class=""details_block vrsupport"">.*<div class=""details_block vrsupport"">(.*)<div class=""details_block vrsupport"">",
                RegexOptions.Compiled);

        private static readonly Regex regVrSupportPlayAreaSection =
            new Regex(
                @"<div class=""details_block vrsupport"">.*<div class=""details_block vrsupport"">.*<div class=""details_block vrsupport"">(.*)",
                RegexOptions.Compiled);

        private static readonly Regex regVrSupportFlagMatch =
            new Regex(
        ...
        private static readonly Regex regLanguageSupport = new Regex(
            @"<td style=""width: 94px; text-align: left"" class=""ellipsis"">\s*([^<]*)\s*<\/td>[\s\n\r]*<td class=""checkcol"">[\s\n\r]*(.*)[\s\n\r]*<\/td>[\s\n\r]*<td class=""checkcol"">[\s\n\r]*(.*)[\s\n\r]*<\/td>[\s\n\r]*<td class=""checkcol"">[\s\n\r]*(.*)[\s\n\r]*<\/td>",
            RegexOptions.Compiled);

        //Platform Support

        private static readonly Regex regPlatformWindows =
            new Regex(@"<span class=""platform_img win""></span>", RegexOptions.Compiled);

        private static readonly Regex regPlatformMac =
            new Regex(@"<span class=""platform_img mac""></span>", RegexOptions.Compiled);

        private static readonly Regex regPlatformLinux =
        ...
        {
            var result = ScrapeStoreHelper(Id);
        ...
        private AppTypes ScrapeStoreHelper(int id)
        {
            ...
            var redirectTarget = -1;

            var oldTime = LastStoreScrape;
        ...
        {
            // Genres
            var m = regGenre.Match(page);
            if (m.Success)
            {
                Genres = new List<string>();
                foreach (Capture cap in m.Groups[2].Captures) Genres.Add(cap.Value);
            }

            // Flags
            var matches = regFlags.Matches(page);
            if (matches.Count > 0)
            {
                Flags = new List<string>();
                foreach (Match ma in matches)
                {
                    var flag = ma.Groups[1].Value;
            ...
            if (matches.Count > 0)
            {
                ...
                foreach (Match ma in matches)
                {
                    var tag = WebUtility.HtmlDecode(ma.Groups[1].Value.Trim());
            ...
            if (m.Success)
            {
                ...
                foreach (Match ma in matches)
                {
                    var headset = WebUtility.HtmlDecode(ma.Groups[1].Value.Trim());
            ...
            if (m.Success)
            {
                ...
                foreach (Match ma in matches)
                {
                    var input = WebUtility.HtmlDecode(ma.Groups[1].Value.Trim());
            ...
            if (m.Success)
            {
                ...
                foreach (Match ma in matches)
                {
                    var playArea = WebUtility.HtmlDecode(ma.Groups[1].Value.Trim());
            ...
            if (matches.Count > 0)
            {
                ...
                foreach (Match ma in matches)
                {
                    var language = WebUtility.HtmlDecode(ma.Groups[1].Value.Trim());
            ...
            if (m.Success)
            {
                ...
                if (!Flags.Contains("Steam Achievements")) Flags.Add("Steam Achievements");
                var num = 0;
                if (int.TryParse(m.Groups[1].Value, out num)) TotalAchievements = num;
            }
            ...
            if (m.Success)
            {
                ...
                foreach (Capture cap in m.Groups[2].Captures) Developers.Add(WebUtility.HtmlDecode(cap.Value));
            }
            ...
            if (m.Success)
            {
                ...
                foreach (Capture cap in m.Groups[2].Captures) Publishers.Add(WebUtility.HtmlDecode(cap.Value));
            }

            // Get release date
            m = regRelDate.Match(page);
            if (m.Success) SteamReleaseDate = m.Groups[1].Captures[0].Value;
            ...
            {
                var num = 0;
                if (int.TryParse(m.Groups[1].Value, out num)) ReviewPositivePercentage = num;
            ...
            if (m.Success) MetacriticUrl = m.Groups[1].Captures[0].Value;
    ...
    public class GameDB
    {
        ...
        {
            if (Games.ContainsKey(id)) return scheme.HasFlag(Games[id].AppType);
            return scheme.HasFlag(AppTypes.Unknown);
        }

        public string GetName(int id)
        {
            if (Games.ContainsKey(id)) return Games[id].Name;
            return null;
        }
        ...
        public List<string> GetGenreList(int gameId, int depth = 3, bool tagFallback = true)
        {
            ...
            {
                var res = Games[gameId].Genres;
                if (tagFallback && (res == null || res.Count == 0))
                {
                    var tags = GetTagList(gameId, 0);
                    if (tags != null && tags.Count > 0) res = new List<string>(tags.Intersect(GetAllGenres()));
                }
        ...
        public bool SupportsVr(int gameId, int depth = 3)
        {
            if (Games.ContainsKey(gameId))
            {
                var res = Games[gameId].VrSupport;
        ...
        public List<string> GetDevelopers(int gameId, int depth = 3)
        {
            if (Games.ContainsKey(gameId))
            {
                var res = Games[gameId].Developers;
                if ((res == null || res.Count == 0) && depth > 0 && Games[gameId].ParentId > 0)
        ...
        public List<string> GetPublishers(int gameId, int depth = 3)
        {
            if (Games.ContainsKey(gameId))
            {
                var res = Games[gameId].Publishers;
                if ((res == null || res.Count == 0) && depth > 0 && Games[gameId].ParentId > 0)
        ...
        public int GetReleaseYear(int gameId)
        {
            if (Games.ContainsKey(gameId))
            {
                var dbEntry = Games[gameId];
                DateTime releaseDate;
                if (DateTime.TryParse(dbEntry.SteamReleaseDate, out releaseDate)) return releaseDate.Year;
            }
        ...
        {
            if (allStoreGenres == null) return CalculateAllGenres();
        ...
        {
            if (allStoreDevelopers == null) return CalculateAllDevelopers();
        ...
        {
            if (allStorePublishers == null) return CalculateAllPublishers();
        ...
        {
            if (allStoreFlags == null) return CalculateAllStoreFlags();
        ...
        {
            var headsets = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);
            var input = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);
            var playArea = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);

            foreach (var entry in Games.Values)
            {
                if (entry.VrSupport.Headsets != null) headsets.UnionWith(entry.VrSupport.Headsets);
                if (entry.VrSupport.Input != null) input.UnionWith(entry.VrSupport.Input);
                if (entry.VrSupport.PlayArea != null) playArea.UnionWith(entry.VrSupport.PlayArea);
            }
        ...
        {
            var Interface = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);
            var Subtitles = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);
            var FullAudio = new SortedSet<string>(StringComparer.OrdinalIgnoreCase);

            foreach (var entry in Games.Values)
            {
                if (entry.LanguageSupport.Interface != null) Interface.UnionWith(entry.LanguageSupport.Interface);
                if (entry.LanguageSupport.Subtitles != null) Subtitles.UnionWith(entry.LanguageSupport.Subtitles);
                if (entry.LanguageSupport.FullAudio != null) FullAudio.UnionWith(entry.LanguageSupport.FullAudio);
            }
        ...
        public IEnumerable<Tuple<string, int>> CalculateSortedDevList(GameList filter, int minCount)
        {
            ...
            var devCounts = new Dictionary<string, int>();
            if (filter == null)
                foreach (var dbEntry in Games.Values)
                    CalculateSortedDevListHelper(devCounts, dbEntry);
            else
                foreach (var gameId in filter.Games.Keys)
        ...
        private void CalculateSortedDevListHelper(Dictionary<string, int> counts, GameDBEntry dbEntry)
        {
            if (dbEntry.Developers != null)
                for (var i = 0; i < dbEntry.Developers.Count; i++)
                {
                    var dev = dbEntry.Developers[i];
        ...
        public IEnumerable<Tuple<string, int>> CalculateSortedPubList(GameList filter, int minCount)
        {
            ...
            var PubCounts = new Dictionary<string, int>();
            if (filter == null)
                foreach (var dbEntry in Games.Values)
        ...
        private void CalculateSortedPubListHelper(Dictionary<string, int> counts, GameDBEntry dbEntry)
        {
            if (dbEntry.Publishers != null)
                for (var i = 0; i < dbEntry.Publishers.Count; i++)
                {
                    var Pub = dbEntry.Publishers[i];
        ...
        {
            var genreNames = GetAllGenres();
            var tagCounts = new Dictionary<string, float>();
            if (filter == null)
                foreach (var dbEntry in Games.Values)
            ...
            float weightFactor, int tagsPerGame)
        {
            ...
            {
                var tagsToLoad = tagsPerGame == 0 ? dbEntry.Tags.Count : Math.Min(tagsPerGame, dbEntry.Tags.Count);
                for (var i = 0; i < tagsToLoad; i++)
                {
                    ...
                    if (weightFactor > 1)
                    {
                        ...
                        {
                            var interp = i / (float) (tagsToLoad - 1);
        ...
        public static XmlDocument FetchAppListFromWeb()
        {
            ...
            var req = WebRequest.Create(@"http://api.steampowered.com/ISteamApps/GetAppList/v0002/?format=xml");
        ...
        public int IntegrateAppList(XmlDocument doc)
        {
            ...
            foreach (XmlNode node in doc.SelectNodes("/applist/apps/app"))
            {
                ...
                {
                    var gameName = XmlUtil.GetStringFromNode(node["name"], null);
        ...
        public int UpdateFromAppInfo(string path)
        {
            ...
            var appInfos = AppInfo.LoadApps(path);
            var timestamp = Utility.GetCurrentUTime();

            foreach (var aInf in appInfos.Values)
            {
                GameDBEntry entry;
        ...
        public int UpdateFromHltb(bool includeImputedTimes)
        {
            ...
            using (var wc = new WebClient())
            {
                ...
                var json = wc.DownloadString(Resources.UrlHLTBAll);
                var parsedJson = JObject.Parse(json);
                dynamic games = parsedJson.SelectToken("Games");
                foreach (var g in games)
                {
                    var steamAppData = g.SteamAppData;
                    int id = steamAppData.SteamAppId;
                    if (Games.ContainsKey(id))
                    {
                        var htlbInfo = steamAppData.HltbInfo;
        ...
        public void ChangeLanguage(StoreLanguage lang)
        {
            ...
            var gamesToUpdate = new Queue<int>();
            if (FormMain.CurrentProfile != null)
            {
                foreach (var game in FormMain.CurrentProfile.GameData.Games.Values)
                    if (game.Id > 0)
                        gamesToUpdate.Enqueue(game.Id);
                var scrapeDlg = new DbScrapeDlg(gamesToUpdate);
        ...
        public void Save(string path, bool compress)
        {
            ...
            try
            {
                ...
                if (compress) stream = new GZipStream(stream, CompressionMode.Compress);
                ...
                var x = new XmlSerializer(typeof(GameDBEntry));
                var nameSpace = new XmlSerializerNamespaces();
                nameSpace.Add("", "");
                foreach (var g in Games.Values) x.Serialize(writer, g, nameSpace);
        ...
        public void Load(string path, bool compress)
        {
            ...
            try
            {
                ...
                if (compress) stream = new GZipStream(stream, CompressionMode.Decompress);
                ...
                var gameListNode = doc.SelectSingleNode("/" + XmlName_RootNode);

                var fileVersion = XmlUtil.GetIntFromNode(gameListNode[XmlName_Version], 0);
                ...
                else
                {
                    ...
                    foreach (XmlNode gameNode in gameListNode.SelectSingleNode(XmlName_Games).ChildNodes)
                    {
                        ...
                        var entry = (GameDBEntry) x.Deserialize(reader);
                        Games.Add(entry.Id, entry);
                    }
                }

                Program.Logger.Write(LoggerLevel.Info,
                    "GameDB XML processed, load complete. Db Language: " + dbLanguage);
            }
            finally
            {
                stream?.Close();
        ...
        private void LoadGamelistVersion1(XmlNode gameListNode)
        {
            ...
            {
                int id;
                if (!XmlUtil.TryGetIntFromNode(gameNode[XmlName_Game_Id], out id) || Games.ContainsKey(id)) continue;
                var g = new GameDBEntry();
                g.Id = id;
