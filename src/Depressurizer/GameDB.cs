...
namespace Depressurizer
{
    ...
    public class GameDBEntry
    {
        ...
        {
            Program.Logger.WriteVerbose(GlobalStrings.GameDB_InitiatingStoreScrapeForGame, id);
            ...
            try
            {
                ...
                while (resp.StatusCode == HttpStatusCode.Found && count < 5)
                {
                    ...
                    if (resp.ResponseUri.ToString() == resp.Headers[HttpResponseHeader.Location])
                    {
                        ...
                        Program.Logger.WriteVerbose(GlobalStrings.GameDB_RedirectsToItself, id);
                        return AppTypes.Unknown;
                    }
                ...
                if (count == 5 && resp.StatusCode == HttpStatusCode.Found)
                {
                    ...
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_TooManyRedirects, id);
                    return AppTypes.Unknown;
                }
                else if (resp.ResponseUri.Segments.Length < 2)
                {
                    // If we were redirected to the store front page
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingRedirectedToMainStorePage,
                        id);
                    SetTypeFromStoreScrape(AppTypes.Unknown);
                    return AppTypes.Unknown;
                }
                else if (resp.ResponseUri.Segments[1] == "agecheck/")
                {
                    ...
                    {
                        // Age check + redirect
                        Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingHitAgeCheck, id,
                    ...
                    else
                    {
                        ...
                        Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingAgeCheckNoRedirect, id);
                        return AppTypes.Unknown;
                    }
                }
                else if (resp.ResponseUri.Segments[1] != "app/")
                {
                    ...
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingRedirectedToNonApp, id);
                    return AppTypes.Other;
                }
                else if (resp.ResponseUri.Segments.Length < 3)
                {
                    // The URI ends with "/app/" ?
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_Log_ScrapingNoAppId, id);
                    return AppTypes.Unknown;
                }
                else if (resp.ResponseUri.Segments[2].TrimEnd('/') != id.ToString())
                {
                    // Redirected to a different app id
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingRedirectedToOtherApp, id,
                ...
                page = sr.ReadToEnd();
                Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingPageRead, id);
            }
            catch (Exception e)
            {
                // Something went wrong with the download.
                Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingPageReadFailed, id, e.Message);
            ...
            {
                Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingReceivedSiteError, id);
            ...
            else if (regGamecheck.IsMatch(page) || regSoftwarecheck.IsMatch(page))
            {
                ...
                {
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingParsedDLC, id,
                        string.Join(",", Genres));
                    result = AppTypes.DLC;
                }
                else
                {
                    Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingParsed, id,
                        string.Join(",", Genres));
            ...
            else
            {
                ...
                Program.Logger.WriteVerbose(GlobalStrings.GameDB_ScrapingCouldNotParse, id);
                result = AppTypes.Unknown;
            }
